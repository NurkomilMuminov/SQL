create database F44_SQL_HOMEWORK_8
go
use F44_SQL_HOMEWORK_8

/* Using Products table, find the total number of products available in each category. */

select Category, Sum(ProductID) Total from Products
group by Category
...
select avg(Price) Elec_Avg_Price from Products
...
select Country ,sum(CustomerID) Total_OF_Customers from Customers
group by Country;

/* Using Orders table, find the minimum and maximum quantity ordered. */

select Min(Quantity) Min_Order, max(Quantity) as Max_Order from Orders


/* Using Orders and Invoices tables, list customer IDs who placed orders in 2023 January
...
/* Using Products and Products_Discounted table, Combine all product names
from Products and Products_Discounted including duplicates. */

select * from Products
select * from Products_Discounted

select * from Products
full join Products_Discounted
on Products.ProductName = Products_Discounted.ProductName

/* Using Products and Products_Discounted table, Combine all product names
from Products and Products_Discounted without duplicates. */

select Products.ProductName, Products_Discounted.ProductName from Products
join Products_Discounted
on Products.ProductName = Products_Discounted.ProductName

/* Using Orders table, find the average order amount by year. */

select YEAR(OrderDate) As [Year], avg(Quantity) as [Avg] from Orders
group by YEAR(OrderDate);

select * from Orders

/* Using Products table, group products based on price: 'Low' (<100), 'Mid' (100-500),
...
/* Using City_Population table, use Pivot to show values of Year column in seperate
...
select ProductID, sum(SaleAmount) as Total_in_Sales from Sales
group by ProductID
...
/* Using City_Population table, use Pivot to show values of City column in seperate
columns (Bektemir, Chilonzor, Yakkasaroy) and copy results to a new Population_Each_City table. */

select * from city_population

select district_id, [Bektemir], [Chilonzor], [Yakkasaroy] 
from
    ( select 
        district_id, 
...
/* Using Invoices table, show top 3 customers with the highest total invoice amount.
Return CustomerID and Totalspent. */

select top 3 TotalAmount from Invoices
select top 3 
...
/* Using Products and Sales tables, list product names and the number of times each has been sold.
(Research for Joins) */
