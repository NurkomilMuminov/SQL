create database F44_SQL_CLASS_7_HW
GO
use F44_SQL_CLASS_7_HW
...
select count(DepartmentName) DepartmentName from Employees
group by DepartmentName

/* Write a query to show the minimum and maximum Price 
of products grouped by Category. Use products table. */

Select Category, max(Price) MAX, min(Price) MIN from Products
group by Category
...
/* Write a query to filter departments having more than 5 employees 
...
/* Write a query to calculate the total sales and average sales 
for each product category from the Sales table. */

select Sum(SaleAmount), avg(ProductID) from Sales
Group by SaleID

/* Write a query to count the number of employees from the Department HR. */

select DepartmentName, Count(Employees.EmployeeID) as Number from Employees
where DepartmentName = 'HR'
group by DepartmentName

/* Write a query that finds the highest and lowest Salary by department 
in the Employees table.(DeptID is enough, if you don't have DeptName). */

select DepartmentName, max(Salary) MAX, min(Salary) MIN from Employees
Group by DepartmentName

/* Write a query to calculate the average salary per Department.
(DeptID is enough, if you don't have DeptName). */

select DepartmentName ,avg(Salary) Average_Salary from Employees
Group by DepartmentName

/* Write a query to show the AVG salary and COUNT(*) of employees working in each department.
(DeptID is enough, if you don't have DeptName). */

select DepartmentName, avg(Salary) Salary, count(*) [ALL] from Employees
group by DepartmentName

/* Write a query to filter product categories with an average price greater than 400. */

select Category, avg(Price) Price from Products
group by Category
...
select * from Employees
select DepartmentName, sum(Salary) Total, avg(Salary) Average from Employees
group by DepartmentName
...
/* Write a query that calculates the total sales and counts unique products sold
...
select year(OrderDate) Year, min(Quantity) Min, MAx(Quantity) max from Orders
