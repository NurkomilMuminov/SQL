...
/* Write a query that uses the WHERE clause to filter for employees 
with Salary >= 60000 and DepartmentName = 'HR'. */
select * from f44_SQL_class_4.dbo.Employees
where Salary >= 60000 and Department = 'HR'
use f44_SQL_class_4
EXEC sp_rename 'Employees.Bonus', 'Salary', 'COLUMN';
select * from f44_SQL_class_4.dbo.Employees

/* Use ISNULL to replace NULL values in the Email column 
with the text "noemail@example.com".From Employees table */
use f44_SQL_class_4_HW
select * from f44_SQL_class_4.dbo.Employees
alter table f44_sql_class_4.dbo.employees
add Email varchar(100)
insert into Employees values (6, 'Beka', 'HR', 65000.00,'example@email.com'), 
(7, 'Spirs', 'IT', 100000.00,'123@email.com'),
(8, 'Bob Dylan', 'Sales', 150000.00, '3445@email.com')

select isnull(Email, 'noemail@example.com') as Email, EmployeeID,EmployeeName 
from f44_SQL_class_4.dbo.Employees 
...
create table Products (ProductID Int Primary key, Category varchar(50), ProductName varchar(100))
insert into Products values 
	(1, 'Home', 'Bread'), (2, 'Garden', 'Scisors'), (3, 'Kitchen', 'Knife'),
	(4, 'Kitchen', 'Fridge')
select distinct category, Productname from Products
select * from Products
/* After SELECT DISTINCT on two columns (Category and ProductName) 
...
select top 10 * from f44_sql_class_3.dbo.Products
order by Price desc

/* Use COALESCE to return the first non-NULL value from FirstName or LastName in the Employees table. */
select coalesce(EmployeeName, Department) as Name from f44_SQL_class_4.dbo.Employees
select * from f44_SQL_class_4.dbo.Employees
/* Write a query that selects distinct Category and Price from the Products table. */ 
select distinct Price, Category from f44_sql_class_3.dbo.Products
alter table f44_sql_class_3.dbo.products
add category varchar(50)
insert into f44_sql_class_3.dbo.Products values (6, 'Knife', 15000.00, 'Kitchen'),
	(4, 'Toaster', 50000.00, 'Kitchen'),
	(5, 'Sugar', 1000.00, 'Kitchen')
select * from f44_sql_class_3.dbo.Products
...
alter table f44_sql_class_4.dbo.employees
add age int
update f44_SQL_class_4.dbo.Employees
...
set age = 35
where EmployeeID = 3
/* Use OFFSET-FETCH to select rows 11 to 20 from 
the Employees table, ordered by Salary DESC. */
select * from f44_sql_class_4.dbo.Employees
order by Salary desc
offset 11 rows fetch next 20 rows only;
/* Write a query to display all products 
with Price <= 1000 and StockQuantity > 50, sorted by Stock in ascending order. */
select * from f44_sql_class_3.dbo.Products
where Price <=1000 and StockQuantity > 50
alter table f44_sql_class_3.dbo.products
add StockQuantity int
update f44_sql_class_3.dbo.Products
set StockQuantity = 100
where ProductID = 3

/* Write a query that filters the Products table 
for ProductName values containing the letter 'e' using LIKE. */
Select * from f44_sql_class_3.dbo.Products
where ProductName like '%e%'

/* Use IN operator to filter for employees 
who work in either 'HR', 'IT', or 'Finance'. */
select * from f44_SQL_class_4.dbo.Employees
where Department in ('HR', 'IT', 'finance')

/* Use ORDER BY to display a list of customers 
ordered by City in ascending and PostalCode in descending order.Use customers table */
create table Customers (ID int primary key, [Name] varchar(50), Department varchar(50),
	City varchar(100), PostalCode varchar(50))
insert into Customers values (1, 'Bob', 'IT', 'USA', 'US-0001'),
	(2, 'Tom', 'HR', 'Canada', 'CA-002'),
	(3, 'Jerry', 'Marketing', 'USA', 'US-0001'),
	(4, 'Bek', 'Accounting', 'UZB', 'UZB-007')
...
/* Write a query that selects the top 5 products with the highest sales,
...
/* Write a query that selects products whose 
Price is less than 10% of the average price in the Products table. 
(Do some research on how to find average price of all products) */
SELECT * FROM Products
WHERE Price < (SELECT AVG(Price) * 1.10 FROM Products);
select avg(price) from Products

/* Use WHERE clause to filter for employees whose 
