create database f44_sql_class_5_HW
go
use f44_sql_class_5_HW
...
/* Write a query that uses an alias to rename the ProductName
column as Name in the Products table. */
select ProductName as Name from Products 

/* Write a query that uses an alias to rename
the Customers table as Client for easier reference. */
select * from Customers as Client

/* Use UNION to combine results from two queries that
select ProductName from Products and ProductName from Products_Discounted. */
select ProductName from Products
union
select ProductName from Products_Discounted

/* Write a query to find the intersection of Products
and Products_Discounted tables using INTERSECT. */
...
/* Write a query that uses CASE to create a conditional
column that displays 'High' if Price > 1000, and 'Low' if Price <= 1000 from Products table. */
select
    case 
 ...
 from Products
 /* Use IIF to create a column that shows 'Yes' if StockQuantity > 100, and 'No'
...
Products and Products_Discounted tables using EXCEPT. */
select * from Products
except
select * from Products_Discounted
...
/* Write a query to find employees in the Employees table
...
/* Use EXCEPT to find customers' ID who have placed orders but do not
have a corresponding recordin the Sales table. */
select CustomerID from Orders
except
select CustomerID from Sales

/* Write a query that uses a CASE statement to determine the discount percentage based on the quantity
...
select CustomerID, Quantity,
    case
        when Quantity = 1 then 'Discount 3%'
        when Quantity between 1 and 3 then 'Discount 5%'
        else 'Discount 7%'
        end As Discount
    from Orders
